...
def main():
    ...
    with movie_writer.saving(fig, args.movie_file, data.num_steps) if should_write_movie else get_dummy_context_mgr():
        for t in range(data.num_steps):
            ...
            plt.plot(sam.states_[0:, 0], sam.states_[0:, 1], 'b')
            map = sam.get_current_map()
            for landmark, covariance in map:
                plot2dcov(landmark, covariance, 'b')
    ...
    # Plot Chi-squared errors
    # fig, ax = plt.subplots()
    # chi_squared_errors = sam.chi_squared_errors_
    # ax.plot(np.arange(0, len(chi_squared_errors), 1), chi_squared_errors)
    # ax.set_title("$\chi^2$-error")
    # ax.set_xlabel("Time")
    # ax.set_ylabel("Error")
    #
    # plt.show(block=True)
    ...
    fig, ax = plt.subplots()
    correct_assoc = sam.data_ass
    ax.plot(np.arange(0, len(correct_assoc), 1), correct_assoc)
    ax.set_title("JCBB data association")
    ax.set_xlabel("Time")
    ax.set_ylabel("Number of correct association")
