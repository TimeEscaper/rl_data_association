...
def main():
    ...
    with env.movie_writer.saving(env.fig, args.movie_file, env.num_steps) if should_write_movie else get_dummy_context_mgr():
        ...
        plt.cla()
        plt.title('Reward (precision) per step')
        plt.ylabel('Ratio of correct data associations')
        plt.xlabel('Time step')
        plt.plot(all_rewards)
        plt.show()
        plt.pause(5)

        print(rewards_1/(rewards_0 + rewards_1 + rewards_half) + 0.7 * rewards_half/(rewards_0 + rewards_1 + rewards_half))
