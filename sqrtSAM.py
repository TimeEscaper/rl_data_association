...
class SqrtSAM(SlamBase):
    def __init__(self, initial_state_mu, initial_state_Sigma, alphas, Q, solver="numpy"):
        ...
        self.accord = {0: 6, 1: 7, 2: 1, 3: 0, 4: 2, 5: 3, 6: 5, 7: 4}
        self.data_ass = []

    def get_jcbb(self, observation):
        N = 4
        ...
        S = np.zeros((2*M,2*M))
        for i in range(M):
            S[2*i:2*(i+1),2*i:2*(i+1)] = self.Q_

        # P = np.zeros((3+4*N, 3+4*N))
        P = 1e-4*np.eye(3+4*N)
        P[0:3,0:3] = self.get_state_covariance_(-1)
        ...
        y = self.get_current_state()
        field_map = FieldMap(N)
        for i in range(2*N):
            y = np.hstack((y, [field_map.landmarks_poses_x[i], field_map.landmarks_poses_y[i]]))
        for i in range(Prop[:,0].size):
            f = np.zeros((2*M, 1))
            G = np.zeros((2*M, (3+4*N)))
            for j in range(M):
                lm_id = int(Prop[i,2*j+1])
                landm = [field_map.landmarks_poses_x[lm_id], field_map.landmarks_poses_y[lm_id]]
                f[2 * j:2 * (j + 1)] = (observation[j, :2] - self.get_observation_(self.get_current_state(), landm)).reshape(2,1)
                dx = field_map.landmarks_poses_x[lm_id] - y[0]
                dy = field_map.landmarks_poses_y[lm_id] - y[1]
                D = (dx**2 + dy**2)**0.5
                L = dy/dx
